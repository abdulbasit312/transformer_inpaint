...
class get_dataset(Dataset):
    def __init__(self, pt_dataset, mask_path=None, test_mask_path=None, is_train=False, image_size=512,mask_prct=0):

        self.is_train = is_train
        self.pt_dataset = pt_dataset
        self.mask_prct=mask_prct
        self.image_id_list = []     
        self.image_id_list=glob(self.pt_dataset + '/*')

        if is_train:    
            self.strokes_mask_list=glob(mask_path+'/*')
    ...
    def load_mask(self, img, index):
        ...
        if self.training is False:  #test
            if self.mask_prct!=0:
                mask_index = random.randint(1000*(self.mask_prct-1)*2, min(1000*(self.mask_prct)*2,len(self.mask_list)-1) )
            else:
    ...
    def load_edge(self, img):
        return canny(img, sigma=2, mask=None).astype(np.float64) 
