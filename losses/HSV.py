...
def BGR2HSV(img, pred_img, edge, batch):
    ...
    img_hsv = np.stack([img_hsv[i] for i in range(batch)],axis=0)
    pre_img_hsv = np.stack([pre_img_hsv[i]  for i in range(batch)],axis=0)
    edge_mask_list = np.stack([edge_mask_list[i]  for i in range(batch)],axis=0)
...
def HSV(img, pred_img, edge):
    ...
    loss = F.mse_loss(hsv, pre_hsv, reduction='none')  # No reduction yet, shape (batch, 3, H, W)
    # Apply edge weighting to each channel separately
    weighted_loss = loss * (1 - edge) + 10 * loss * edge  # Emphasize edges

    # Split losses by channel
    losses_H = weighted_loss[:, 0, :, :].mean()  # Loss for H channel
    losses_S = weighted_loss[:, 1, :, :].mean()  # Loss for S channel
    losses_V = weighted_loss[:, 2, :, :].mean()  # Loss for V channel

    # Total loss is the sum of all channel losses
    total_loss = losses_H + losses_S + losses_V

    return losses_H, losses_S, losses_V, total_loss
