...
def compare_ssim(pairs):
    ...
    return structural_similarity(real, fake,data_range=1.0, channel_axis=2)


# ================================


def mae(reals, fakes, num_worker=2):
    error = 0
...
def psnr(reals, fakes, num_worker=2):
    error = 0
    pool = Pool(num_worker)
...
def ssim(reals, fakes, num_worker=2):
    error = 0
    pool = Pool(num_worker)
...
def fid(reals, fakes, num_worker=2, real_fid_path=None):
    dims = 2048
    batch_size = 6
    ...
    if os.path.isfile(real_fid_path):
        ...
        reals = (np.array(reals).astype(np.float32) ).transpose((0, 3, 1, 2))
    ...
    fakes = (np.array(fakes).astype(np.float32)).transpose((0, 3, 1, 2))
